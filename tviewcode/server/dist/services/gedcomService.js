...
class GedcomService {
     ...
     */
    async parseGedcomFile(filePath) {
        try {
     ...
     */
    async importToDatabase(gedcomData) {
        try {
            const stats = {
                individuals: 0,
                families: 0,
            ...
            // Process individuals
            const individualMap = new Map(); // Map GEDCOM IDs to database IDs
     ...
     */
    async createPersonFromGedcom(individual) {
        // Extract name(s)
        const names = [];
        if (individual.name) {
            names.push({
        ...
        if (individual.events) {
            for (const gedcomEvent of individual.events) {
                ...
                if (gedcomEvent.tag === 'BIRT' || gedcomEvent.tag === 'DEAT')
                    continue;
                try {
     ...
     */
    async createEventFromGedcom(gedcomEvent, personId) {
        ...
        const eventType = (eventTypeMap[gedcomEvent.tag] || 'Custom');
     ...
     */
    async createRelationshipsFromFamily(family, individualMap) {
        const relationships = [];
