...
class ExportService {
    ...
    async exportAllData(exportDir) {
        try {
            ...
            fs.writeFileSync(path.join(exportDir, 'persons.json'), JSON.stringify(personsData, null, 2));
            ...
            fs.writeFileSync(path.join(exportDir, 'events.json'), JSON.stringify(eventsData, null, 2));
            ...
            fs.writeFileSync(path.join(exportDir, 'relationships.json'), JSON.stringify(relationshipsData, null, 2));
            stats.relationships = relationships.length;
            // Export media
            const media = await Media.find();
            const mediaData = await this.processMedia(media, exportDir);
            fs.writeFileSync(path.join(exportDir, 'media.json'), JSON.stringify(mediaData, null, 2));
     ...
     */
    async processMedia(media, exportDir) {
     ...
     */
    async exportPerson(personId, exportDir) {
        try {
            // Create export directory if it doesn't exist
            if (!fs.existsSync(exportDir)) {
                fs.mkdirSync(exportDir, { recursive: true });
